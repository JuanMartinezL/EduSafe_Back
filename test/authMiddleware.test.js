...
describe('Middleware de autorización', () => {
  ...
  beforeAll(async () => {
    ...
    // Crear usuarios
    adminUser = await User.create({ name: 'Admin', last_name: 'Admin', email: 'admin@test.com', password: '123456', role: adminRole._id });
    studentUser = await User.create({ name: 'Student', last_name: 'One', email: 'student@test.com', password: '123456', role: studentRole._id });
  });
  ...
  beforeEach(async () => {
    // Limpia la colección de roles antes de cada prueba
    await Role.deleteMany({});
  });
